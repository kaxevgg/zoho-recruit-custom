...
window.rec_embed_js = window.rec_embed_js || {
	...
	show_hide_rec_loading: function (listingDiv, properties, isHide) {
		...
		if (isHide) {
			...
			var loadigElem = rec_embed_js.create_element({
				"name": "div",
				"attrs": {
					"class": "cw-easyapply-loading"
				},
				"child": [ //NO I18N
					{
						"name": "div",
	...
	"print_facets": function (widget_id, data, facets) { //NO I18N
		...
		data.forEach(function (each_data) {
			facetSysRefArr.forEach(function (api_name) {
				...
				var content = each_data[api_name],
					contentArr;
		...
		var widget_facet_elem = rec_embed_js.create_element({
			"name": "div", //NO I18N
			"attrs": {
				"class": "rec_facet_group",
		...
		widget_facet_elem.addEventListener("click", function (event) { //NO I18N
			...
			if (target.nodeName === "INPUT" && target.type === "checkbox") {
				...
				var widget_data = window[closest_ul.dataset.widgetid + "_data"],
					properties = window[closest_ul.dataset.widgetid + "_properties"];
		...
		facets.forEach(function (eachFacet) {
			...
			if (facetValuesObj && Object.keys(facetValuesObj).length > 0) {
				var ulElement = rec_embed_js.create_element({
					"name": "ul",
					"attrs": {
						"class": "rec_ul_filter",
						"data-sysrefname": eachFacet.api_name
					}
				}); //NO I18N

				Object.keys(facetValuesObj).sort().forEach(function (eachFacetValue) {
					var li = rec_embed_js.create_element({
						"name": "li",
						"attrs": {
							"data-value": eachFacetValue
						},
						"child": [ //NO I18N
							{
								"name": "label",
								"attrs": {
									"class": "cw-facet-checkbox-label"
								},
								"child": [ //NO I18N
									{
										"name": "input",
										"attrs": {
											"type": "checkbox",
											"class": "cw-facet-checkbox"
										}
									}, //NO I18N
									{
										"name": "span",
										"text": (eachFacet.api_name === 'Remote_Job' ? eachFacet.field_label : eachFacetValue) + " (" + facetValuesObj[eachFacetValue] + ")"
									} //NO I18N
								]
							}
						]
				...
				if (eachFacet.api_name === 'Remote_Job') {
					mainDiv = rec_embed_js.create_element({
						"name": "div",
						"child": [ //NO I18N
							{
								"name": "div",
	...
	"load": function (properties) { //NO I18N
		...
		var successCallback = function (response, properties) {
			...
			var data = response.data,
				facets = response.info && response.info.facets; //NO I18N
	...
	"print_jobs": function (jobs, resp_info, opted_filter, properties) { //NO I18N
		var empty_grouping_value = [],
			job_groups = {},
			filterKeys = opted_filter && Object.keys(opted_filter),
			//groupingFldName = resp_info.facets ? "" : resp_info && resp_info.groups && resp_info.groups[0].api_name;
			groupingFldName = resp_info && resp_info.groups && resp_info.groups[0].api_name,
		...
		for (var eachInd = 0; eachInd < jobs.length; eachInd++) {
			...
			if (filterKeys && filterKeys.length) {
				...
				for (var filterIndex = 0; filterIndex < filterKeys.length; filterIndex++) {
					var eachFilter = filterKeys[filterIndex],
						jobVal = eachJob[eachFilter],
						jobVal = jobVal && typeof jobVal === "object" ? jobVal.name : jobVal;
		...
		var job_listing_elem = listingDiv.querySelector("." + widget_id + "_jobs") ? listingDiv.querySelector("." + widget_id + "_jobs") : rec_embed_js.create_element({
			"name": "div",
			"attrs": {
				"class": widget_id + "_jobs"
			}
		}); //NO I18N
		...
		if (Object.keys(job_groups).length === 0 && empty_grouping_value.length === 0) {
			...
			var groupElem = rec_embed_js.get_group_jobs(job_groups, resp_info),
				otherElem = rec_embed_js.get_group_jobs({
					"Others": empty_grouping_value
				}, resp_info, true); //NO I18N
			if (groupElem !== "") {
				job_listing_elem.appendChild(rec_embed_js.get_group_jobs(job_groups, resp_info));
			}
			if (otherElem !== "") {
				job_listing_elem.appendChild(rec_embed_js.get_group_jobs({
					"Others": empty_grouping_value
				}, resp_info, true)); //NO I18N
			}
		}
	...
	"get_group_jobs": function (job_groups, resp_info, no_grouping) { //NO I18N
		...
		var jobListingFrg = document.createDocumentFragment(),
			grouping_values = Object.keys(job_groups).sort(),
			dummyDivElem = document.createElement("div");
		for (var eachGroupIndex = 0; eachGroupIndex < grouping_values.length; eachGroupIndex++) {
			var groupName = grouping_values[eachGroupIndex],
				groupValues = job_groups[groupName],
				groupUl = rec_embed_js.create_element({
					"name": "ul",
					"attrs": {
						"class": "rec-group"
					}
			...
			if (!no_grouping) {
				jobListingFrg.appendChild(rec_embed_js.create_element({
					"name": "h2",
					"attrs": {
						"class": "rec-grp-heading"
					},
					"text": groupName + " ( " + groupValues.length + " ) "
				})); //NO I18N
			}
			...
			for (var eachJobIndex = 0; eachJobIndex < groupValues.length; eachJobIndex++) {
				var eachJob = groupValues[eachJobIndex], //NO I18N 
					...
					jobInfoUl = rec_embed_js.create_element({
						"name": "ul",
						"attrs": {
							"class": "rec-job-info"
						},
				...
				for (var fldInd = 0; fldInd < resp_info.fields.length; fldInd++) {
					var eachField = resp_info.fields[fldInd],
